...
import AtteptError from "../src/utils/error/AttemptError.js";
...
describe("예외 처리", () => {
  describe("자동차 예외 테스트", () => {
    test("자동차 이름 중복 예외 테스트", () => {
      ...
      }).toThrow(new CarError(ERROR_MESSAGE.CAR_NAME_DUPLICATE));
    });
    ...
    test("자동차 개수 예외 처리", () => {
      ...
      expect(() => {
        validateCars(cars, carNames);
      }).toThrow(new CarError(ERROR_MESSAGE.CAR_MAX_COUNT));
    });
  });
  ...
  describe("시도 횟수 예외 처리", () => {
    test("시도 횟수가 숫자가 아닌 경우 예외 처리", () => {
      ...
      }).toThrow(new AtteptError(ERROR_MESSAGE.INVALID_ATTEMPT_NUMBER));
    });

    test("시도 횟수가 음수인 경우 예외 처리", () => {
      expect(() => {
        validateAttempt(-1);
      }).toThrow(new AtteptError(ERROR_MESSAGE.ATTEMPT_TOO_LOW));
    });

    test("시도 횟수가 50회를 초과할 경우 예외 처리", () => {
      expect(() => {
        validateAttempt(51);
      }).toThrow(new AtteptError(ERROR_MESSAGE.ATTEMPT_TOO_HIGH));
    });
  });
});
